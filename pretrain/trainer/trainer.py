...
class Trainer:
    ...
    def generate_anchor(self, atom1, atom2, si_data_batch, s_data_batch, weights, group_data_batch, sub_batch_idx, mask_indices=None):
        ...
        # m1 = torch.rand(select1.shape).to(self.device) > 0.2
        # m2 = torch.rand(select1.shape).to(self.device) > 0.2
        # select1 = select1 & m1
        # select2 = select2 & m2
        ...
        anchor_subgraph2 = self.model.re_gnn_forward(anchor_subgraph, s_data_batch.edge_index[:, select2], s_data_batch.edge_attr[select2, :], weights)
        ...
        for epoch in range(1, num_epoch + 1):
            ...
            if epoch % self.interval == 0:
                ckpt = {'gnn':self.model.gnn.state_dict(),
                        ...
                        'w_l':self.model.w_l.state_dict(),
                        'w_v':self.model.w_v.state_dict()}
