...
from utils import smi2mol, mol2smi, get_submol
from utils import cnt_atom, MAX_VALENCE, dfs
...
class Tokenizer:
    ...
    def tokenize(self, mol):
        ...
        # piece_idxs = [,self.subgraph_to_idx(x[0]) for x in res]
        group_idxs = [x[1] for x in res]
        return group_idxs#, group_idxs
...
class BricsTokenize:
    ...
    def tokenize(self, mol):
        ...
        b, e = [], []
        res = list(BRICS.FindBRICSBonds(rdkit_mol))

        break_bond = set()

        for break_e in res:
            begin, end = break_e[0][0], break_e[0][1]
            break_bond.add((begin, end))

        # print(break_bond)
        for bond in rdkit_mol.GetBonds():
            a1 = bond.GetBeginAtom().GetIdx()
            a2 = bond.GetEndAtom().GetIdx()
            if ((a1, a2) in break_bond) or ((a2, a1) in break_bond):
                continue
            b.append(a1)
            e.append(a2)
        ...
        adj = torch.zeros((mol.GetNumAtoms(), mol.GetNumAtoms()), dtype=torch.long)
        adj[b, e] = 1
        adj = adj.tolist()
...
if __name__ == '__main__':
    ...
    # tokenizer = BricsTokenize()
    tokenizer = Tokenizer('/data00/jiahao/mole_supervise/ps/zinc250_table_size300.txt')
    r = BRICS.BRICSDecompose(smi2mol('S=[Se]=S'))
    print(r)
    print(tokenizer('S=[Se]=S'))
