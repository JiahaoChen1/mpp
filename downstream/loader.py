...
from tokenizer import Tokenizer, BricsTokenize
from utils import subgraph_connect, subgraph_inner_connect, to_onehot#, drop_nodes, subgraph_connect_single
import time
...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        if not empty:
            ...
            self.smiles = pd.read_csv(os.path.join(self.processed_dir,'smiles.csv'), header=None)[0].tolist()
        
        # if os.path.exists(os.path.join(self.processed_dir, 'process.pkl')):
            self.subgraphs = torch.load(os.path.join(self.processed_dir, 'process.pkl'))

        # self.tokenizer = Tokenizer(vocab_path=vocab_path)
    def get(self, idx):
        data = Data()
        ori_data = Data()
        for key in self.data.keys:
            ...
            item, slices = self.data[key], self.slices[key]
            s = list(repeat(slice(None), item.dim()))
            s[ori_data.__cat_dim__(key, item)] = slice(slices[idx], slices[idx + 1])
            ori_data[key] = item[s]

        groups, subgraph_data, subgraph_inn_data, subgraph_weights = \
                self.subgraphs[0][idx], self.subgraphs[1][idx], self.subgraphs[2][idx], self.subgraphs[3][idx]
        
        # ori_data = copy.deepcopy(data)
        # check = AllChem.MolFromSmiles(smile)
        # check = mol_to_graph_data_obj_simple(check)

        # # print(smile, self.smiles[idx])
        
        # assert check.x.tolist() == data.x.tolist()
        # assert check.edge_index.T.tolist() == data.edge_index.T.tolist()
        # assert check.edge_attr.tolist() == data.edge_attr.tolist()

        subgraph_edge = torch.tensor(np.array(subgraph_data[0]).T, dtype=torch.long)
        subgraph_attr = torch.tensor(np.array(subgraph_data[1]), dtype=torch.long)
        subgraph_data = Data(x=torch.zeros(max(groups) + 1, 1), edge_index=subgraph_edge, edge_attr=subgraph_attr) 
        
        # subgraph_inn_atom = torch.tensor(np.array(atom_feature_list).reshape(-1, 1), dtype=torch.long)
        subgraph_inn_edge = torch.tensor(np.array(subgraph_inn_data[0]).T, dtype=torch.long) 
        subgraph_inn_attr = torch.tensor(np.array(subgraph_inn_data[1]), dtype=torch.long)


        # if data.x.shape[0] != 1 and data.edge_index.shape[1] != 0:
        #     if len(subgraph_inn_edge.shape) == 1:
        #         assert data.edge_index.shape[1] == subgraph_edge.shape[1]
        #     elif len(subgraph_edge.shape) == 1:
        #         assert data.edge_index.shape[1] == subgraph_inn_edge.shape[1]
        #     else:
        #         assert data.edge_index.shape[1] ==  subgraph_inn_edge.shape[1] + subgraph_edge.shape[1]
        # data.x = torch.cat([data.x, subgraph_inn_atom], dim=1)
        data.edge_index, data.edge_attr = subgraph_inn_edge, subgraph_inn_attr

        data.groups = groups
        data.subgraph_weights = subgraph_weights
        return ori_data, data, subgraph_data
    ...
    def process(self):
        ...
        if 'brics' in self.vocab_path:
            self.tokenizer = BricsTokenize()
            print('use brics')
        else:
        ...
        for idx, smil in enumerate(data_smiles_list):
            groups = self.tokenizer(smil)
            one_hot_groups = to_onehot(groups)
            sub_group_list.append(groups)
            check_smile.append(smil)
            one_hot_group_list.append(one_hot_groups)

        for idx, l_data in enumerate(data_list):
            s_edges, s_attr, _ = subgraph_connect(sub_group_list[idx], l_data.edge_index, l_data.edge_attr)
            si_edges, si_attr = subgraph_inner_connect(sub_group_list[idx], l_data.edge_index, l_data.edge_attr)
            sub_data_list.append((s_edges, s_attr))
            subi_data_list.append((si_edges, si_attr))
        ...
        self.subgraphs = (one_hot_group_list, sub_data_list, subi_data_list, subgraph_weights)

        torch.save((data, slices), self.processed_paths[0])
        torch.save((one_hot_group_list, sub_data_list, subi_data_list, subgraph_weights), os.path.join(self.processed_dir, 'process.pkl'))
...
def _load_bbbp_dataset(input_path):
    ...
    for idx, m in enumerate(preprocessed_rdkit_mol_objs_list):
        if m != None:
            preprocessed_smiles_list.append(smiles_list[idx])
        else:
            preprocessed_smiles_list.append(None)
...
if __name__ == "__main__":
    ...
    m = MoleculeDataset(root='/data00/jiahao/mole_supervise/dataset/tox21', vocab_path='/data00/jiahao/mole_supervise/ps/zinc250_table_size100.txt', dataset="tox21")

    d = DataLoader(m, batch_size=32, shuffle=False, num_workers = 4)
    t1 = time.time()
    for idx, item in enumerate(d):
        ...
        t2 = time.time()
    print(t2 - t1)
        # print(torch.tensor(eval(edge)).dtype)
        # break
    # print(m[0])
