...
def connect(group1, group2, dic, edge_list, edge_feature_list, i, j, atom_feature_list):
    for n1 in group1:
        for n2 in group2:
            if (n1, n2) in dic:
                ...
                atom_feature_list[n1] = 1
                atom_feature_list[n2] = 1
...
def subgraph_connect(groups, mole_edge, attr):
    ...
    edge_list = []
    edge_feature_list = []
    atom_feature_list = [0 for _ in range(max([item for sublist in groups for item in sublist]) + 1)]
    ...
    return edge_list, edge_feature_list, atom_feature_list
...
def children(atom, edges):
    r = []
    for i in range(len(edges[atom])):
        if edges[atom][i] == 1:
            r.append(i)
    return r

def dfs(edges, root, collect = []):
    collect.append(root)
    t = children(root, edges)
